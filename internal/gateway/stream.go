...
// event, in order.
// It is kept separate from the HTTP plumbing so it can be unit-tested with a
// synthetic stream and a capturing emit.
func translateStream(body io.Reader, real string, emit func(event string, data any)) {
	...
	// Blocks open lazily and run sequentially: reasoning_content opens
	...
	// the buffer is data i took from the anthropic doocs, not sure where
	...
	for sc.Scan() {
		...
		for _, tc := range d.ToolCalls {
			...
			if !seen {
				bidx = startBlock(blkTool, map[string]any{
					"type": "tool_use",
