...
// If the message carries any image block, it returns a []oaiPart multimodal
// array instead: text blocks become {type:"text"} parts and image blocks
// become {type:"image_url"} parts. Base64 sources are wrapped as a
...
func (s *Server) toOpenAI(a anthReq) (string, oaiReq) {
	...
	max := a.MaxTokens
	if max == 0 {
		max = 4096 // not magic number, number from OpenAI
	}
	...
	// Anthropic effort -> OpenAI reasoning effort
...
// converts a non-streaming OpenAI response into the Anthropic message
// object: a text block first (if any), then a tool_use block
...
func buildMessageResponse(o oaiResp, real string) map[string]any {
	...
	return map[string]any{
		"id":            firstNonEmpty(o.ID, newMsgID()),
		"type":          "message",
		"role":          "assistant",
		"model":         real,
		"content":       content,
